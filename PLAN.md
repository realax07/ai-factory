...
| merge-qa-branches | ПМ | Merge qa-веток: r2-qa-api-tests + r2-qa-web-tests + bug002 (G8 вердикты на руках) | закрыто | e22c7e5/ca21f25/40feb35 в main; конфликт test_r2_fast2.py разрешен версией багфикса (xfail снят); flow_check OK, openspec 9/9; G8 api-return (TC-cat-008) — фикс следующим шагом |
| bug002-review-final | code_reviewer | Ревью BUG-002 (перезапуск завершился) | закрыто | APPROVE (review-001.md): смок 8/8, регресс 116/0, xfail снят подтвержден; 2 minor; рекомендация: стенды на явно заданных портах (дефолт 8080 пересекся с чужой сессией) — кандидат в BACKLOG |
| g8-fix-cat008 | qa_automation | G8 return-фикс: TC-cat-008 → xfail(strict) + BUG-003 репорт (deleg_61855f45) | закрыто | 80da775 в main: дословные ассерты восстановлены, 2 xfail strict (XPASS при фиксе продукта), BUG-003 (Should: тело ошибки); 115 passed/2 xfailed; push origin/main |
| r2-final-regression | qa_automation | Финальный полный регресс R2 (deleg_e30a1e57) | закрыто | api ЗЕЛЕНЫЙ 115/9 skip/2 xfail; web КРАСНЫЙ: BUG-004 — продуктовый регресс от BUG-002 (форма шлет явный null, сервер честно 422; TC-UI-011/018 падают); seed-нюанс archive-теста (seed до прогона); чекбокс 6.2 честно не отмечен; боевая БД не тронута (проверено) |
| r2-bug004-dev | dev | Багфикс BUG-004: UI fast подстановка high + блокировка поля (4.3) (deleg_5d8a32ba) | закрыто | 2411b99 в origin/feature/bug004-fast-ui-priority, диф 2 файла +47/−5, web 45/0 (TC-UI-011/018 зеленые), API-инварианты целы; замечание: stale env стендов чистить (кандидат в BACKLOG) |
| r2-bug004-review | code_reviewer | Ревью BUG-004 фикса (deleg_25e4223b) | закрыто | APPROVE (review-001.md): web 45/0 (TC-UI-011/018 зеленые), UI-смоук playwright, API-инварианты целы, a11y disabled корректен; 2 minor |
| merge-bug004 | ПМ | Merge BUG-004 fix в main | закрыто | ee40bac review + 41340c5 merge; flow_check OK; push origin/main |
| r2-final-check | qa_automation | Контрольный полный регресс на main 41340c5 (deleg_7f19b041) | закрыто | ЗЕЛЕНЫЙ: web 45/0, api 115/0/2xfail; TC-UI-011/018 passed; чекбокс 6.2 отмечен; аномалия: пустая БД без seed дает 1 средовый fail (докстринг TC-env-001); коммит 38651ee за ПМ |
| r2-archive | sa | Архивация пакета add-r2-categories-settings (контракт 7: дельты → master-spec, перенос в archive) (deleg_72846c90) | закрыто | openspec 10/10 (было 8: +categories, +settings; fastline MODIFIED слит, дублей нет), flow_check OK, пакет в archive/2026-09-22 (git mv, история сохранена); вопрос sdd.md — см. решение ПМ |
| archive-sdd-decision | ПМ | Решение: корневой sdd.md | закрыто | РЕШЕНО: sdd.md пакета (в archive) достаточен как документированная дельта; обновление корневого sdd.md — задача следующего релиза/СА (кандидат в BACKLOG, S-объем) |
| r2-release-closed | ПМ | РЕЛИЗ 2 ЗАКРЫТ: все задачи tasks.md [x], ворота зеленые, пакет архивирован | закрыто | main 85e8432: web 45/0, api 115/0/2xfail; BUG-002/003/004 задокументированы (BUG-003 xfail strict до фикса продукта); готов к деплою |
| r2-deployed | ПМ | Релиз 2 задеплоен Заказчиком (deploy-r2.sh) + хотфикс 541e844 (кеш-бастинг ?v=static_v, кнопки V3) принят Заказчиком визуально | закрыто | дефекты найдены Заказчиком при просмотре: DEF-005 (формы vs V3 — открыт, Флоу 2); прод доступен по https://IP:10443 (self-signed), nginx :10443 → uvicorn :8377 |
...
| r2-tests-g8-review | qa_case_reviewer | G8: ревью кода автотестов обеих веток (deleg_76536fd2) | закрыто | review-002-tests.md: web ОДОБРИТЬ (3 minor, 45/0 с первого прогона); api ВЕРНУТЬ (1 major: TC-cat-008 ассерт ослаблен до подстроки, маскирует расхождение sdd↔каркас "validation error"; 4 minor) — фикс после ревью BUG-002 |
...
## Релиз 2 — итоговая сводка (закрыт 2026-09-23)

- **Состав:** справочник категорий (P5), настройки (P2), форма задачи (P1: селекты, автодополнение, priority-lock, крестик, визуал V3 «Бумага» на всю доску), DEF-001 закрыт
- **Цикл:** ba → sa (10 spec) → 5 задач × [dev → review] → qa-контур (impact H5 → чеклист CHK-82..147 → 66 кейсов → ревью → автотесты 116 API + 45 web) → 3 багфикса (BUG-002/003/004) → архивация (контракт 7)
- **Роли:** 12 задействованных (в т.ч. новые ui_designer, qa_impact_analyst — оба с первого боя полезны)
- **Параллелизм:** 6 параллельных запусков, worktree-изоляция; инцидентов пересечения зон после введения worktree — 0
- **Найдено багами:** BUG-002 (null-обход priority-lock, Must, закрыт), BUG-003 (каркасный 422-формат, Should, xfail), BUG-004 (UI-регресс от BUG-002, закрыт)
- **Коммиты ключевые:** requirements 8bfd01b, пакет a8caab5, задачи 909478d / ab537b1+ca21f25 / de769f4+db89106, qa-сьюты 40feb35-цепочка, багфиксы da782f5 / 2411b99, архив 85e8432
